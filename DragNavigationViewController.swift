...
open class DragNavigationViewController: UINavigationController {
    ...
    var isEnabledPopGestureRecognizer: Bool = true {
        didSet {
            ...
            customNavigationTransactionHelper.isEnableGestureRecognizer = isEnabledPopGestureRecognizer
        }
    }
...
// MARK: CustomNavigationTransactionHelper
final class CustomNavigationTransactionHelper: NSObject {
    ...
    private weak var navigationController: DragNavigationViewController?
    ...
    func setup(_ navigationController: DragNavigationViewController) {
...
private final class PushPopAnimator: NSObject, UIViewControllerAnimatedTransitioning {
...
extension View {
    ...
    func setupWithNavigationGesture(position: Int) -> some View {
        self.modifier(CustomNavigationModifier(position: position))
    }
}

// MARK: CustomNavigationModifier
struct CustomNavigationModifier: ViewModifier {
    ...
    func body(content: Content) -> some View {
        content
            .introspect(.scrollView, on: .iOS(.v15, .v16, .v17, .v18)) { view in
                guard let collectionView = view as? UICollectionView else { return }
                let gestureRecognizers = collectionView.gestureRecognizers?.filter({
                    $0 is UIPanGestureRecognizer
                })
                let startPosition = 0
                let isDisable = position == startPosition
                collectionView.bounces = !isDisable
                gestureRecognizers?.forEach { it in
                    it.view?.tag = isDisable ? CustomNavigationTransactionHelper.enableScrollViewTag : 0
                }
            }
    }
}
